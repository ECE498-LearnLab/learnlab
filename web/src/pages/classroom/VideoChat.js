...
const VideoChat = ({ room, participants, dominantSpeaker, screenShareParticipant, onLeaveRoomHandler}) => {
  const [isLocalAudioEnabled, toggleIsLocalAudioEnabled] = useLocalAudioToggle(room.localParticipant)
  ...
  const remoteParticipants = participants.map(participant => (
    <RemoteParticipant key={participant.identity} participant={participant} isLocalParticipant={false} />
  ))
      ...
      <Card style={styles.videoWrapper}>
        ...
        <CardBody>
          <div style={styles.videoContainer}>
            ...
            />
            <Animated animationIn="fadeIn" animationOut="fadeOut" isVisible={isAllParticipantsVisible}>
              <div className="m-2" style={styles.otherParticipantsContainer}>
                <div className="hideScrollBar" style={styles.otherParticipants}>
                  <RemoteParticipant key={room.localParticipant.identity} participant={room.localParticipant} isLocalParticipant={true} isLocalAudioEnabled={isLocalAudioEnabled} />
                  {remoteParticipants}
                </div>
              </div>
            </Animated>
            <div style={styles.actionContainer}>
              ...
              <div>
                <Fab onClick={toggleIsLocalAudioEnabled} className="bg-primary text-white m-2" color="inherit" size="medium">
                  {isLocalAudioEnabled ? <Mic /> : <MicOff />}
                </Fab>
                <Fab onClick={toggleScreenShare} disabled={isSharing} className="bg-primary text-white m-2" color="inherit" size="medium">
                  <Monitor />
                </Fab>
                <Fab onClick={onLeaveRoomHandler} className="bg-white text-danger m-2" color="inherit" size="medium">
                  <X />
                </Fab>
              </div>
              <div className="m-3">
